...
class TwinSVM(nn.Module):
    def __init__(self):
        super(TwinSVM, self).__init__()
...
def train(X, Y, model, args):
    ...
    for epoch in range(args.epoch):
        ...
        for i in range(0, N, args.batchsize):
            ...
            loss = tsvm_loss(output, y, weight, args.c)
            loss.backward()
            optimizer.step()
...
def visualize(X, Y, model):
    ...
    plt.contourf(x, y, z, levels=[-1, 0, 1], colors=['pink', 'cyan', 'red'], alpha=0.6)
